...
class ErrorHandlingTester {
    ...
    const result = await this.runTest(
      ...
      async () => {
        const response = await apiClient.get(
          "/api/youtube/videos?action=channel",
        );
  ...
  async testRetryMechanism() {
    const result = await this.runTest(
      ...
      async () => {
        ...
        const response = await apiClient.get(
          "http://invalid-domain-that-does-not-exist.com/api/test",
          {
            retries: 2,
            retryDelay: 100,
          },
        ...
        if (response.status !== 0 && response.status !== 500) {
          throw new Error(
            `Expected status 0 or 500 for network error, got ${response.status}`,
          );
        }
      },
    );
  ...
  async testValidationErrors() {
    const result = await this.runTest("Validation Error Handling", async () => {
      // Test invalid action parameter
      const response = await apiClient.get(
        "/api/youtube/videos?action=invalid",
      );

      if (response.status !== 400) {
        throw new Error(
          `Expected 400 status for validation error, got ${response.status}`,
        );
      }
  ...
  async testErrorMessageFormatting() {
    const result = await this.runTest("Error Message Formatting", async () => {
    ...
    const result = await this.runTest(
      ...
      async () => {
        ...
        if (successful === 0 && rateLimited === 0) {
          throw new Error(
            "No requests completed successfully or hit rate limit",
          );
        }
